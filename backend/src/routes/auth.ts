...
import { hashEmail, maskEmail } from '../utils/hash';
...
router.post('/send-otp', async (req: Request, res: Response) => {
  ...
  await prisma.oTP.upsert({
    where: { email_hash: emailHashVal },
    update: { code: otp, expiresAt },
    create: { email: emailHashVal, email_hash: emailHashVal, code: otp, expiresAt },
  });
...
router.post('/register', async (req: Request, res: Response) => {
  ...
  const otpRecord = await prisma.oTP.findUnique({ where: { email_hash: emailHashVal } });
  if (!otpRecord || otpRecord.code !== otp || new Date() > otpRecord.expiresAt) {
    return res.status(400).json({ error: 'OTP invalid sau expirat' });
  }

  // Check duplicate — by hash and username only
  const existing = await prisma.user.findFirst({
    where: { OR: [{ email_hash: emailHashVal }, { username }] },
  });
  if (existing) return res.status(409).json({ error: 'Email sau username deja folosit' });

  // Build secure fields (only if encryption key is configured)
  const secureFields = config.emailEncryptionKey
    ? { email_encrypted: encryptEmail(email), email_hash: emailHashVal, email_display: maskEmail(email) }
    : {};
  ...
  return res.status(201).json({ token, user: { ...publicUser, email, league: ratingToLeague(user.rating) } });
});
...
router.post('/login', async (req: Request, res: Response) => {
  ...
  // Find user — hash only
  const user = await prisma.user.findUnique({ where: { email_hash: emailHashVal } });
  ...
  const token = jwt.sign({ userId: user.id }, config.jwtSecret, { expiresIn: config.jwtExpiresIn } as jwt.SignOptions);
  const { email_encrypted, email_hash, email: _ef, ...publicUser } = user;
  return res.json({ token, user: { ...publicUser, email, league: ratingToLeague(user.rating) } });
});
